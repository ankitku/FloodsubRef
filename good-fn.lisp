...
;;           :pro
;;           (:claim (pending-!in-seen-s-fn (cdr s)))
;;           (:claim (pending-!in-seen-s-fn (set-subs-sfn nbrs topics p (cdr s))))
...
;;              (set-subs-psfn nbrs topics (car (car s))
;;                             p (cdr (car s))))
;;           (= (mget :pending (set-subs-psfn nbrs topics (car (car s))
;;                                            p (cdr (car s))))
;;              (mget :pending (cdr (car s))))
;;           (:claim (pending-!in-seen-ps-fn (cdr (car s))
;;                                           (mget :pending (cdr (car s)))))
;;           (:prove :hints (("Goal" :use ((:instance
;;                                          prop=pending-!in-seen-set-nsubs-psfn
;;                                          (p (car (car s)))
;;                                          (st (cdr (car s)))
;;                                          (ms (mget :pending (cdr (car
;;                                                                   s)))))))))
;;           :prove))
...
;;   (:pro :induct :bash
;;         :pro
;;         (:claim (pending-!in-seen-s-fn (leave-fn p (cdr s))))
;;         (:claim (!= (car (car s)) p))
;;         (:claim (== (leave-fn p s)
;;                     (cons (car s) (leave-fn p (cdr s)))))
;;         (:claim (s-fnp (cons (car s) (leave-fn p (cdr s)))))
;;         (:r pending-!in-seen-s-fn) :s
;;         (:equiv (leave-fn p s)
;;                 (cons (car s) (leave-fn p (cdr s))))
                
;;         (= (cdr (car (cons (car s) (leave-fn p (cdr s)))))
;;            (cdr (car s)))
;;         :prove :prove :prove))
...
(in-theory (enable ordered-seenp))
